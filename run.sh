...
if [[ $(ps U snarayan | grep flask | wc -l) > 1 ]]; then
    pid=$(ps U snarayan | grep flask | grep -v grep | awk '{ print $1 }')
...
python -m flask run --host=0.0.0.0 > /local/snarayan/logs/doxy.log 2>&1 & disown
echo " Done"
